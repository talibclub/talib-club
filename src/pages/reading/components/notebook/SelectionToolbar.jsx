...
import { Crop, ScanText, Type, ChevronsUp, ChevronsDown, FileStack, GitBranch, Spline, Network, Trash2, Check, Copy } from 'lucide-react';
    ...
    >
      ...
      {(onBranch || onConnect) && (
        <>
          ...
          {onAutoLayout && (
            <button style={btn} onClick={onAutoLayout} title="จัดระเบียบมายแมพ (จากจุดนี้)">
      ...
      {divider}
      {onCopy && <button style={btn} onClick={onCopy} title="คัดลอก (Ctrl+C)"><Copy size={17} strokeWidth={1.7} /></button>}
      <button style={btn} onClick={onDuplicate} title="ทำซ้ำ (Ctrl+D)"><FileStack size={17} strokeWidth={1.7} /></button>
      <button style={{ ...btn, color: '#EF4444' }} onClick={onDelete} title="ลบ (Del)"><Trash2 size={17} strokeWidth={1.7} /></button>
